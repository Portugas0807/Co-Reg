...
class eurosat_dataloader():
    def __init__(self, batch_size, num_workers, root_dir, log, args):
        self.root_dir = os.path.join(root_dir, "eurosat")
        ...
        eurosat_dataset = dsets.EuroSAT(self.root_dir, download=True)
        temp_train, temp_test = self.split_train_test_dataset(eurosat_dataset)
    ...
    def get_image_files_and_labels(self, dataset):
        image_files = [dataset.samples[i][0] for i in range(len(dataset))]
    ...
    def split_train_test_dataset(self, dataset):
        ...
        class train_test_split(dsets.EuroSAT):
            def __init__(self, root, idx):
                super(train_test_split, self).__init__(root)

...
class svhn_dataloader():
    def __init__(self, batch_size, num_workers, root_dir, log, args):
        self.root_dir = os.path.join(root_dir, "svhn")
        ...
        class MySVHN(dsets.SVHN):
            def __init__(self, root, split):
                super(MySVHN, self).__init__(root, split=split, download=True)
                self.classes = ["number 0", "number 1", "number 2", "number 3", "number 4", "number 5",
                                "number 6", "number 7", "number 8", "number 9"]

        temp_train = MySVHN(self.root_dir, 'train')
        temp_test = MySVHN(self.root_dir, 'test')

        self.train_data = temp_train.data
        self.train_label = temp_train.labels
        self.test_data = temp_test.data
        self.test_label = temp_test.labels

        # transform train/test data to cifar-style

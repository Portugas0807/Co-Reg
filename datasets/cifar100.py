...
class split_dataset(Dataset):
    def __init__(self, train_data, train_label, partialY, noise_label, test_data, test_label, transform, mode, transform2=None, pred=[], probability=[], pred_score=None, log='', args=None):
        """
        ...
        if self.mode == "labeled":
            ...
            one_hot_pseudo_label = torch.zeros(num_unlabeled, self.num_class).cuda().scatter_(1, pseudo_label.unsqueeze(1), 1)
            ...
            in_candidate_pseudo_acc = torch.eq(pseudo_label, train_label_of_unlabeled_split).int()[in_candidate_pred_idx].sum() / num_in_candidate_pred * 100.0
            out_candidate_pseudo_acc = torch.eq(pseudo_label, train_label_of_unlabeled_split).int()[out_candidate_pred_idx].sum() / num_out_candidate_pred * 100.0
    ...
    def __getitem__(self, index):
        if self.mode == 'warmup':
            ...
            img, target, prob, pred_score = self.train_data[index], self.partialY[index], self.probability[index], self.pred_score[index]
            img = Image.fromarray(img)
...
class cifar100_dataloader():
    def __init__(self, batch_size, num_workers, root_dir, log, args):
        self.root_dir = os.path.join(root_dir, "cifar100")
        ...
        temp_train = dsets.CIFAR100(self.root_dir, train=True, download=True)
        temp_test = dsets.CIFAR100(self.root_dir, train=False, download=True)

        if args.data_limits:
            idx = [i for i in range(len(temp_train))]
            train_idx = random.sample(idx, args.used_samples)

            class MyCIFAR100(dsets.CIFAR100):
                def __init__(self, root, split, idx):
                    super(MyCIFAR100, self).__init__(root, train=split, download=True)
                    self.data = self.data[idx]
                    self.targets = [self.targets[i] for i in idx]

            temp_train = MyCIFAR100(self.root_dir, True, train_idx)
        ...
        if args.noisy_type == "clip":
            ...
            dlabels_train = np.array(self.train_label.cpu()).astype('int')
            ...
            self.partialY = torch.tensor(np.array(self.partialY).astype('float')).float().cuda()
    ...
    def run(self, mode, pred=[], prob=[], pred_score=None, args=None):
        if mode == 'warmup':
            all_dataset = split_dataset(self.train_data, self.train_label, self.partialY, self.noise_label, self.test_data, self.test_label, transform=self.strong_transform, mode="warmup", log=self.log, args=args)
            ...
            labeled_dataset = split_dataset(self.train_data, self.train_label, self.partialY, self.noise_label, self.test_data, self.test_label, transform=self.weak_transform, mode="labeled", transform2=self.strong_transform, pred=pred, probability=prob, pred_score=pred_score, log=self.log, args=args)
            ...
            unlabeled_dataset = split_dataset(self.train_data, self.train_label, self.partialY, self.noise_label, self.test_data, self.test_label, transform=self.weak_transform, mode="unlabeled", transform2=self.strong_transform, pred=pred, probability=prob, pred_score=pred_score, log=self.log, args=args)
            ...
            test_dataset = split_dataset(self.train_data, self.train_label, self.partialY, self.noise_label, self.test_data, self.test_label, transform=self.transform_test, mode='test', log=self.log, args=args)
            ...
            eval_dataset = split_dataset(self.train_data, self.train_label, self.partialY, self.noise_label, self.test_data, self.test_label, transform=self.weak_transform, mode='eval_train', log=self.log, args=args)

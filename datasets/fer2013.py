...
class MyFER2013(dsets.FER2013):
    def __init__(self, root, split):
        super(MyFER2013, self).__init__(root, split=split)
        self.classes = ["Angry Facial Expression", "Disgust Facial Expression", "Fear Facial Expression",
                        "Happy Facial Expression", "Sad Facial Expression", "Surprise Facial Expression",
                        "Neutral Facial Expression"]


class fer2013_dataloader():
    def __init__(self, batch_size, num_workers, root_dir, log, args):
        self.root_dir = os.path.join(root_dir, "fer2013")
        ...
        fer2013_dataset = MyFER2013(self.root_dir, split='train')
        temp_train, temp_test = self.split_train_test_dataset(fer2013_dataset)
    ...
    def split_train_test_dataset(self, dataset):
        idx = [i for i in range(len(dataset))]
        train_idx = random.sample(idx, int(len(dataset) * self.train_split_ratio))
        test_idx = [i for i in idx if i not in train_idx]

        class train_test_split(MyFER2013):
            def __init__(self, root, idx):
                super(train_test_split, self).__init__(root, split='train')
                self._samples = [self._samples[i] for i in idx]

        return train_test_split(self.root_dir, train_idx), train_test_split(self.root_dir, test_idx)
